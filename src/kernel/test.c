...
static _Noreturn void ext4_vfs_truncate_powercut(bool growing)
{
    ...
    ext4_vfs_require(phipfs_close(file), "truncate cut writer close");
    if (phipfs_fstat(file, &held) != PHIPFS_STATUS_STALE_HANDLE ||
        phipfs_fstat(reader, &held) != PHIPFS_STATUS_STALE_HANDLE)
        kernel_test_fail("ext4 truncate cut retained closed handles");
    ext4_vfs_require(phipfs_sync(volume), "truncate cut final sync");
    ext4_vfs_require(phipfs_unmount(volume), "truncate cut clean unmount");
    if (!phipfs_resources_released() || !ext4_backend_resources_released() ||
        !nvme_filesystem_session_resources_released() || heap_verify() != HEAP_STATUS_OK ||
        paging_verify() != PAGING_STATUS_OK)
...
static _Noreturn void ext4_vfs_xattr_powercut(bool removing)
{
    ...
    const bool present = initial == PHIPFS_STATUS_OK && count == sizeof(expected);
    if (!present && (initial != PHIPFS_STATUS_NOT_FOUND || count != 0U))
        kernel_test_fail("ext4 xattr cut state is neither absent nor committed");
    if (present) {
        ext4_vfs_require(phipfs_get_xattr(volume, name, "user.cut", actual, sizeof(actual), &count), "xattr cut initial value");
    ...
    count = 99U;
    if (removing) {
        if (phipfs_get_xattr(volume, name, "user.cut", actual, sizeof(actual), &count) != PHIPFS_STATUS_NOT_FOUND || count != 0U)
...
_Noreturn void kernel_test_complete_ext4_recovery(void)
{
    ...
    if (ext4_geometry_refusal_test) {
        ...
        if (phipfs_open(PHIPFS_VOLUME_SYSTEM, "system/README.TXT", PHIPFS_ACCESS_READ, &handle) !=
            ...
            phipfs_create(PHIPFS_VOLUME_SYSTEM, "data/user/refused-sector") != PHIPFS_STATUS_NOT_MOUNTED ||
