...
int main(void)
{
    ...
    enum phipia_ext4_test_storage_kind failure_kind = PHIPIA_EXT4_TEST_STORAGE_WRITE;
    ...
    assert(!fail_test_storage_operation(PHIPIA_EXT4_TEST_STORAGE_WRITE));
    assert(!fail_test_storage_operation(PHIPIA_EXT4_TEST_STORAGE_FLUSH));
    assert(ext4_backend_test_finish_storage_probe(&attempts, &failure_kind));
    assert(attempts == 2U && failure_kind == PHIPIA_EXT4_TEST_STORAGE_KIND_COUNT);
    assert(!ext4_backend_test_finish_storage_probe(&attempts, &failure_kind));
    ...
    assert(!fail_test_storage_operation(PHIPIA_EXT4_TEST_STORAGE_WRITE));
    assert(ext4_backend_test_finish_storage_probe(&attempts, &failure_kind));
    assert(attempts == 2U && failure_kind == PHIPIA_EXT4_TEST_STORAGE_FLUSH);
